...
Math docs for every cipher: `docs/math-formulas/`. Cryptanalysis reference (methods, keyspace, gaps): [`docs/cryptanalysis/`](docs/cryptanalysis/). Full variable inventory: [`docs/variable-inventory.md`](docs/variable-inventory.md). Ground truth links ciphers → math → datasets → property profiles.
